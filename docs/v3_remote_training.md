...
- hard cross-entropy только по истинному классу трубки, без label smoothing и
  без добавления соседних трубок в loss;
- первые две трубки используются как seed: первый учитываемый в loss прогноз
  строится для третьей трубки трека.
...
Checkpoint модели, обученной с neighbor smoothing и одним seed-хитом, также не
следует использовать для resume: изменились и целевая функция, и выравнивание
последовательности. Новый эксперимент нужно начинать с нуля.
...
Первые два физических хита трека используются как входной seed. Поэтому первый
учитываемый прогноз имеет `prediction_step=1` и `target_hit_number=3`.
